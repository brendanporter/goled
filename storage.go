...
func getImages() []string {
	...
	for name, p := range images {
		...
		img2html := "<div class='imgContainer card text-white bg-dark mb-3'><div class='card-header'><b style='font-size:18px;'>" + name + "</b><i class='fas fa-times close-btn' onclick=\"deleteImage('" + name + "')\"></i></div><div class='card-body'><img src=\"data:image/png;base64," + imgBase64Str + "\" onclick=\"loadImageToCanvas('" + name + "')\" /></div></div>"
		...
		img2html := fmt.Sprintf(`
			<div class='animContainer card text-white bg-dark mb-3'>
				<div class='card-header'>
					<b style='font-size:18px;'>%s</b><i class='fas fa-times close-btn' onclick="deleteAnimation('%s')"></i>
				</div>
				<div class='card-body'>
